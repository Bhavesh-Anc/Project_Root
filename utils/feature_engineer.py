...
from .feature_engineer import HORIZONS  # Relative import
...
def _single_monte_carlo_simulation(returns_mean, returns_std, days, seed=None):
    if seed is not None:
        np.random.seed(seed)
    return np.random.normal(returns_mean, returns_std, days).cumsum()[-1]
...
class StockEvaluator:
        ...
        for horizon in long_term_horizons:
            try:
                ...
                for ticker, df in self.data.items():
                    ...
                    tasks.append((ticker, mean, std, horizon_days, df['Close'].iloc[-1]))
                    ...
                    with ProcessPoolExecutor() as executor:
                        future_to_ticker = {
                            executor.submit(
                                ...
                                mean, std, horizon_days, price, n_simulations
                            ): ticker for (ticker, mean, std, horizon_days, price) in tasks
                        }
                        for future in as_completed(future_to_ticker):
                            ticker = future_to_ticker[future]
                            try:
                                final_returns = future.result()
                                ...
                                warnings.warn(f"Monte Carlo failed for {ticker}: {str(e)}")
                else:
                    for ticker, mean, std, horizon_days, price in tasks:
                        try:
                            final_returns = self._simulate_prices(mean, std, horizon_days, price, n_simulations)
                            all_simulations.append({
                                'ticker': ticker,
                                'mean_return': np.mean(final_returns),
                                'p5': np.percentile(final_returns, 5),
    ...
    @staticmethod
    def _simulate_prices(mean, std, horizon_days, start_price, n_simulations):
